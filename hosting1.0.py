...
def update_memory(key, value):
    """Store information in conversation memory for personalization"""
    st.session_state["conv_state"]["conversation_memory"][key] = value

def get_memory(key):
    """Retrieve information from conversation memory"""
    return st.session_state["conv_state"]["conversation_memory"].get(key)
...
def smart_response(prompt_template, **kwargs):
    ...
    try:
        ...
        enhanced_template = f"""
        You are a friendly, helpful customer service representative for {COMPANY_NAME}, specializing in {COMPANY_DOMAIN}.        
        
        IMPORTANT GUIDELINES:
        - Be warm and conversational, like talking to a friend
        - Keep responses concise and to the point
        - Avoid repetitive "thank you" phrases
        - When asking for verification info, explain it's "for verification purposes"  
        - Provide single, complete responses (not multiple follow-ups)
        - Use natural, human language - avoid robotic phrases
        - Use contractions (I'll, you're, we've) to sound more natural
        - Ask follow-up questions that show genuine interest
        - Reference previous parts of conversation when relevant
...
def get_products_by_category(category):
    """Get products by category (e.g., chimney, hood)"""
...
if st.session_state["conv_state"]["phase"] == "entry" and not st.session_state["messages"]:
    ...
    try:
        ...
        - Mention that you can help them.
        - Warm invitation to share how you can help
        - Keep it conversational and friendly (1 sentences max)
...
def handle_user_input(text):
    ...
    if phase == "identify_customer":
        ...
        if "guest" in lower_text:
            response = f"Perfect! You're continuing as a guest. How can I help you with your kitchen chimney needs today?\n\n{show_main_menu()}"
            append_message("assistant", response)
    ...
    # PHASE: VERIFY EXISTING CUSTOMER
        ...
        if "guest" in text.lower():
            response = f"No problem! You're now continuing as a guest. How can I help you with your kitchen chimney needs today?\n\n{show_main_menu()}"
            append_message("assistant", response)
            state["phase"] = "main_menu"
            return
        ...
        if email_match:
            ...
            if customer:
                ...
                welcome_response = f"Welcome back, {customer['name']}! {orders_info}How can I assist you today?\n\n{show_main_menu()}"
    ...
    # PHASE: HANDLE NEW CUSTOMER
        ...
        if "guest" in text.lower():
            response = f"No problem! You can continue as a guest. How can I help you with your kitchen chimney needs today?\n\n{show_main_menu()}"
            append_message("assistant", response)
            state["phase"] = "main_menu"
            return
    ...
    # PHASE: CONFIRM CUSTOMER
        ...
        if any(word in text.lower() for word in ["yes", "correct", "right", "that's me"]):
            ...
            response = f"Great! Welcome back {customer['name']}. How can I help you today?\n\n{show_main_menu()}"
            append_message("assistant", response)
            state["phase"] = "main_menu"
        else:
    ...
    # PHASE: COLLECT REGISTRATION
        ...
        if email:
            ...
            response = f"Welcome to Faber, {name}! Your account has been created. How can I help you today?\n\n{show_main_menu()}"
            append_message("assistant", response)
            state["phase"] = "main_menu"
        else:
    ...
    # PHASE: MAIN MENU - Service Selection
        ...
        exit_phrases = ["no thanks", "nothing else", "that's all", "goodbye", "bye", "no thank you", "nope thanks"]
        if any(phrase in text.lower().strip() for phrase in exit_phrases) or text.lower().strip() in ["no", "nope", "nothing"]:
    ...
    # PHASE: CONVERSATION ENDED
        ...
        if any(trigger in text.lower() for trigger in restart_triggers):
            ...
            response = f"Welcome back! How can I help you with your Faber kitchen chimney today?\n\n{show_main_menu()}"
            append_message("assistant", response)
            state["phase"] = "main_menu"
        else:
    ...
    # Handle any unmatched phases
        ...
        response = f"Let me help you get back on track.\n\n{show_main_menu()}"
        append_message("assistant", response)
        state["phase"] = "main_menu"
...
def handle_product_info(text):
    ...
    if text.strip().isdigit():
        response = "I'd be happy to help with product information! Are you interested in kitchen chimneys, range hoods, or exhaust solutions? Or would you like to browse our complete [Product Catalog](https://faber.in/products)?"
        append_message("assistant", response)
        state = st.session_state["conv_state"]
        state["phase"] = "product_query"
        return
    
    # Check for specific product categories and store preference
    if any(word in text.lower() for word in ["chimney", "chimneys"]):
        update_memory("preferred_products", "kitchen_chimneys")
        products_list = get_products_by_category("chimney")
        
        if products_list and len(products_list) <= 3:
            products_text = "\n".join([
                f"• **{product['name']}** ({product['sku']}) - {product['description']}" 
                for product in products_list
            ])
            response = f"Here are our kitchen chimneys:\n\n{products_text}\n\nWhich one interests you, or would you like detailed specs for any specific model?"
        else:
            response = "We have several kitchen chimney models available. For detailed specifications and comparison, please check our [Product Catalog](https://faber.in/chimneys). If you have a specific model in mind, just let me know!"
    ...
    elif any(word in text.lower() for word in ["hood", "hoods"]):
        update_memory("preferred_products", "range_hoods")
        response = "Range hoods are great for efficient kitchen ventilation! For our complete range of hoods with specifications, visit [Hood Collection](https://faber.in/hoods). Which specific features are you looking for?"
        append_message("assistant", response)
        state = st.session_state["conv_state"]
        state["phase"] = "product_query"
        return
    ...
    if product:
        ...
        **{name}** ({sku})
        ...
        Would you like more technical specifications or have questions about this model? Is there anything specific you'd like to know?
        """,
        name=product["name"],
        sku=product["sku"],
        ...
        if "show all" in text.lower() or "all products" in text.lower():
            response = "For our complete product range with detailed specifications and comparisons, please visit our [Product Catalog](https://faber.in/products). If you have a specific product name or model number, I can provide detailed information about it!"
            append_message("assistant", response)
            return_to_menu()
        else:
            # Handle vague/unclear product queries with hyperlinks
    # Handle vague/unclear product queries with hyperlinks
            if any(word in text.lower() for word in ["products", "all", "everything", "what do you have", "show me", "available", "options", "models"]):
                response = f"I'd love to help you explore our products! Since you're looking for general information, I'd recommend browsing our [Complete Product Catalog](https://faber.in/products) where you can see all models with detailed specs and comparisons.\n\nOnce you find something that catches your eye, just tell me the specific model name or features you're interested in, and I can give you targeted information!"
                append_message("assistant", response)
                return_to_menu()
            else:
                response = f"I couldn't find specific information about '{text}'. You might want to browse our [Product Catalog](https://faber.in/products) to see what's available, then let me know which specific model interests you!\n\nAlternatively, you could tell me what type of kitchen setup you have or specific features you're looking for, and I can recommend the right products."
                append_message("assistant", response)
                return_to_menu()
...
def handle_product_query_phase(text):
    ...
    if any(word in text.lower() for word in ["back", "menu", "cancel"]):
        ...
        append_message("assistant", f"{response}\n\n{show_main_menu()}")
        state["phase"] = "main_menu"
        return
